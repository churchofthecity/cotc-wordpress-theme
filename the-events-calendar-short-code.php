...
class Events_Calendar_Shortcode
{
	...
	public function ecs_fetch_events( $atts )
	{
		...
		$atts = shortcode_atts( array(
			...
			'thumbwidth' => '95',
			'thumbheight' => '95',
			'contentorder' => 'thumbnail, title, date, excerpt, venue',
			'event_tax' => '',
			...
			foreach( $posts as $post ) :
				...
				foreach ( $atts['contentorder'] as $contentorder ) {
					switch ( trim( $contentorder ) ) {
						...
						case 'thumbnail' :
							if( self::isValid($atts['thumb']) && has_post_thumbnail() ) {
						...
						case 'date' :
							...
							if( self::isValid($atts['eventdetails']) && !$news_only ) {
								$output .= '<span class="duration time">';
								$output .= apply_filters( 'ecs_event_list_details', tribe_events_event_schedule_details(), $atts );
								$output .= '</span>';
							}
							break;
	...
	private function get_excerpt( $limit, $source = null )
	{
		...
		// $excerpt .= '<p class="read-more"><a href="' . tribe_get_event_link() . '" rel="bookmark">Read More...</a></p>';
		return $excerpt;
	}
}
